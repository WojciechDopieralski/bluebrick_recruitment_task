...
game_is_promo <- data_column_choice %>%
  ...
  mutate(
    ...
    winter_sales = as.factor(ifelse(date > '2016-12-21' & date < '2017-01-03',
                                    "ws1",
                                    ifelse(date > '2017-12-20' & date < '2018-01-04',
                                           "ws2",
                                           "no_ws")))
  ) %>%
...
test <- count_param(sales_period, net_sales_usd, winter_sales, region)

```

# Custom themes
1. theme for waffle chart.
2. Theme for siple line plot that will be converted to plotly objects. 
...
basic_line_season(sum_promo_rev)
...
# Some tests
1. We'll chech if sales distribution (number of net sales) are identical in both seasons
...
filter_data <- date_group %>%
  filter(winter_sales != "no_ws")

wilcox.test(sum_net_sales ~ winter_sales, data=filter_data) 


ggplot(filter_data, aes(x = sum_net_sales, group = winter_sales)) +
  geom_boxplot()
...
ret_obj <- plotly_ojb %>% 
  layout(title=list(text='<b> Sales Season </b>'))
